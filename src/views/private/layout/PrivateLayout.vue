...
export default {
  setup() {
    ...
    const menuConfig = [
      ...
      },
      {
        ...
        items: [
          ...
          { label: "Vínculo representante", path: "/app/rep-infractores", permission: "/rep-infractores", iconSvg: icons.link },
        ...
        items: [
          {
            label: "Usuario",
            path: "/app/usuario",
            permissionAny: ["/usuario", "/usuario/perfil", "/auth/register", "/perfil", "/endpoints"],
            iconSvg: icons.link,
          },
          {
            label: "Reporteria",
            path: "/app/reporteria",
            permissionAny: ["/reporteria", "/reporteria/demografico", "/reporteria/matriz"],
            iconSvg: icons.link,
          },
        ],
      },
    ...
    const tieneAcceso = (endpoint) => {
      ...
      const exact = permisos.find((perm) => normalizar(perm.endpoint) === ep);
      if (exact && exact.VIEW === true) return true;

      return permisos.some((perm) => {
        const p = normalizar(perm.endpoint);
        return perm.VIEW === true && p.startsWith(ep + "/");
      });
    };
    ...
    const canShowItem = (item) => {
      ...
      if (item.permissionAny) return tieneAccesoAAlguno(item.permissionAny);
      if (!item.permission) return true;
      return tieneAcceso(item.permission);
    };

    const shouldShowGroup = (group) => {
      return group.items.some((item) => canShowItem(item));
    };
    ...
    const routeAnyPermissions = (ruta) => {
      if (ruta === "/app/usuario" || ruta.startsWith("/app/usuario/")) {
        return ["/usuario", "/usuario/perfil", "/auth/register", "/perfil", "/endpoints"];
      }
      if (ruta === "/app/reporteria" || ruta.startsWith("/app/reporteria/")) {
        return ["/reporteria", "/reporteria/demografico", "/reporteria/matriz"];
      }
      return null;
    };
    ...
    const verificarSeguridadRuta = () => {
      ...
      const anyPerms = routeAnyPermissions(ruta);
      if (anyPerms) {
        if (!tieneAccesoAAlguno(anyPerms)) router.push("/app/dashboard");
        return;
      }
