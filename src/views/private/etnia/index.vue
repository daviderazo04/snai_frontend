  ...
  <div class="page-container">
    <div class="page-header">
      <div class="title-wrap">
        <h2>Etnias</h2>
        <span class="subtitle">Gestión del catálogo de etnias</span>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="etnia in etnias" :key="etnia.id">
              <td class="id-cell">#{{ etnia.id }}</td>
              <td>
                <span class="name">{{ etnia.nombre }}</span>
              </td>
              <td class="text-right">
                <button 
                  class="btn-icon" 
            ...
            <tr v-if="etnias.length === 0">
              <td colspan="3">
                <div class="empty">
                  ...
                  <span>No hay etnias registradas en el sistema.</span>
                </div>
              </td>
            </tr>
          </tbody>
...
const load = async () => {
  try {
    ...
    // Mantenemos tu lógica original de acceso a datos
...
/* Estilos de Tabla (Reutilizados del componente anterior) */
.table-card {
  background: white;
  border-radius: 12px;
