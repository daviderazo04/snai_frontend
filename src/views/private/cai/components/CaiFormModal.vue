    ...
    <div v-if="open" class="backdrop" @click.self="$emit('close')">
      <div class="modal">
        <header class="modal-header">
          <div>
            <p class="eyebrow">Administración de Centros</p>
            <h3>
              {{ mode === "edit" ? "Editar CAI" : "Nuevo CAI" }}
            </h3>
          </div>
        ...
        <div v-if="loadingCantones" class="loading-state">
          <div class="spinner"></div>
          <p>Cargando lista de cantones...</p>
        </div>

        <form v-else class="modal-body" @submit.prevent="submit">
          <div class="section-title">Información del Centro</div>

          <div class="form-row">
            <label class="field">
              <span>Nombre del CAI</span>
              <input v-model="form.nombre" type="text" placeholder="Ej: CAI Virgilio Guerrero" required />
            </label>

            <label class="field">
              <span>Cantón</span>
              <select v-model="form.cantonId" required>
                <option value="" disabled>Seleccione un cantón...</option>
                <option v-for="canton in listaCantones" :key="canton.id" :value="String(canton.id)">
                  {{ canton.nombre }}
                </option>
              </select>
            </label>
...
// Importamos el servicio de cantones para asegurar la lista completa
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Carga de la lista completa de cantones
    const loadCantones = async () => {
      if (listaCantones.value.length > 0) return;
      loadingCantones.value = true;
      try {
        const res = await getCantones({ size: 1000 });
        listaCantones.value = res.data?.data || res.data || [];
      } catch (error) {
        console.error("Error cargando cantones en modal CAI:", error);
      } finally {
    ...
    const syncForm = () => {
      if (props.initialData) {
        form.value = {
          ...
          cantonId: props.initialData.cantonId ? String(props.initialData.cantonId) : "",
        };
      } else {
    ...
    const submit = () => {
      emit("save", {
        ...
        cantonId: Number(form.value.cantonId),
      });
    };
...
.modal {
  width: min(680px, 100%); background: white; border-radius: 20px;
